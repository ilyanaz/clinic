...
$has_baseline = false;
$latest_annual_test = null; // Store the latest annual test record to display annual values
...
if (!empty($patient_id)) {
    try {
        ...
        // Check if patient has an existing baseline audiometric test
        // The FIRST test record (oldest by date) for each patient is ALWAYS the baseline
        // Priority: 1) Records with baseline_date set, 2) Oldest examination_date
        
        // First, try to find a record with baseline_date explicitly set (like Lisa Tan's 2020-01-15)
        $stmt = $clinic_pdo->prepare("
        ...
        // If no explicit baseline found, use the FIRST/OLDEST test record as baseline
        ...
        // If found, treat it as baseline
        if ($existing_baseline) {
            $has_baseline = true;
            // Ensure baseline_date is set to the examination_date (first test is always baseline)
            if (empty($existing_baseline['baseline_date']) && !empty($existing_baseline['examination_date'])) {
                $existing_baseline['baseline_date'] = $existing_baseline['examination_date'];
            }
            
            // Fetch the latest annual test record (most recent test that is NOT the baseline)
            // This will be used to display annual values if they exist
            $baseline_id = $existing_baseline['id'] ?? null;
            
            if ($baseline_id) {
                // Get the most recent test that is NOT the baseline
                // An annual test is identified by:
                // 1. Has annual_date set, OR
                // 2. Has annual values (annual_right_250, annual_left_250, etc.) set, OR
                // 3. Is a different record than the baseline (different id)
                $stmt = $clinic_pdo->prepare("
                    SELECT * FROM audiometric_tests 
                    WHERE patient_id = ? 
                    AND id != ?
                    AND (
                        ...
                        OR annual_right_250 IS NOT NULL 
                        OR annual_left_250 IS NOT NULL
                        OR annual_right_bone_250 IS NOT NULL
                        OR annual_left_bone_250 IS NOT NULL
                    )
                    ORDER BY 
                ...
                $latest_annual_test = $stmt->fetch(PDO::FETCH_ASSOC);
                
                if ($latest_annual_test) {
                    error_log("Latest annual test found for patient_id $patient_id: Test ID " . ($latest_annual_test['id'] ?? 'N/A') . 
                             ", Examination Date: " . ($latest_annual_test['examination_date'] ?? 'N/A') . 
                             ", Annual Date: " . ($latest_annual_test['annual_date'] ?? 'N/A') .
                             ", Annual Right 250: " . ($latest_annual_test['annual_right_250'] ?? 'NULL') .
                             ", Annual Left 250: " . ($latest_annual_test['annual_left_250'] ?? 'NULL'));
                } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['audiometric_form'])) {
    try {
        ...
        // The FIRST test record (oldest by date) is always the baseline
        ...
        if ($has_existing_baseline) {
            ...
            // Insert baseline values (copied from baseline) + annual data
            $stmt = $clinic_pdo->prepare("
            ...
            // Insert both baseline and annual data
            $stmt = $clinic_pdo->prepare("
                INSERT INTO audiometric_tests (
                    patient_id, surveillance_id, examination_date, audiometer, calibration_date, jkkp_approval_no,
                    seg_value, otoscopy, rinne_right, rinne_left, weber_center, weber_right, weber_left,
        ...
        <form method="POST" id="audiometricForm">
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="ear_infections">
                                ...
                                <option value="YES" <?php echo (isset($latest_annual_test['ear_infections']) && $latest_annual_test['ear_infections'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['ear_infections']) && $latest_annual_test['ear_infections'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="head_injury">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['head_injury']) && $latest_annual_test['head_injury'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['head_injury']) && $latest_annual_test['head_injury'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="ototoxic_drugs">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['ototoxic_drugs']) && $latest_annual_test['ototoxic_drugs'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['ototoxic_drugs']) && $latest_annual_test['ototoxic_drugs'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="previous_ear_surgery">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['previous_ear_surgery']) && $latest_annual_test['previous_ear_surgery'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['previous_ear_surgery']) && $latest_annual_test['previous_ear_surgery'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="previous_noise_exposure">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['previous_noise_exposure']) && $latest_annual_test['previous_noise_exposure'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['previous_noise_exposure']) && $latest_annual_test['previous_noise_exposure'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="significant_hobbies" rows="2" placeholder="Enter significant hobbies"><?php echo htmlspecialchars($latest_annual_test['significant_hobbies'] ?? ''); ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <input type="date" class="form-control" name="examination_date" value="<?php 
                            if ($latest_annual_test && !empty($latest_annual_test['examination_date'])) {
                                echo htmlspecialchars($latest_annual_test['examination_date']);
                            } else {
                                echo date('Y-m-d');
                            }
                        ?>" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" name="audiometer" value="<?php echo htmlspecialchars($latest_annual_test['audiometer'] ?? ' '); ?>" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Calibration Date:</label>
                        <input type="date" class="form-control" name="calibration_date" value="<?php 
                            if ($latest_annual_test && !empty($latest_annual_test['calibration_date'])) {
                                echo htmlspecialchars($latest_annual_test['calibration_date']);
                            } else {
                                echo ' ';
                            }
                        ?>" required>
                ...
                <div class="row mt-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" name="jkkp_approval_no" value="<?php 
                            if ($latest_annual_test && !empty($latest_annual_test['jkkp_approval_no'])) {
                                echo htmlspecialchars($latest_annual_test['jkkp_approval_no']);
                            } else {
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" class="form-control" name="seg_value" placeholder="Enter SEG value" value="<?php echo htmlspecialchars($latest_annual_test['seg_value'] ?? ''); ?>">
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="otoscopy" required>
                                <option value="">-- Select --</option>
                                <option value="NORMAL" <?php echo (isset($latest_annual_test['otoscopy']) && $latest_annual_test['otoscopy'] == 'NORMAL') ? 'selected' : ''; ?>>NORMAL</option>
                                <option value="ABNORMAL" <?php echo (isset($latest_annual_test['otoscopy']) && $latest_annual_test['otoscopy'] == 'ABNORMAL') ? 'selected' : ''; ?>>ABNORMAL</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="rinne_right">
                                <option value="">-- Select --</option>
                                <option value="POSITIVE" <?php echo (isset($latest_annual_test['rinne_right']) && $latest_annual_test['rinne_right'] == 'POSITIVE') ? 'selected' : ''; ?>>POSITIVE</option>
                                <option value="NEGATIVE" <?php echo (isset($latest_annual_test['rinne_right']) && $latest_annual_test['rinne_right'] == 'NEGATIVE') ? 'selected' : ''; ?>>NEGATIVE</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="rinne_left">
                                <option value="">-- Select --</option>
                                <option value="POSITIVE" <?php echo (isset($latest_annual_test['rinne_left']) && $latest_annual_test['rinne_left'] == 'POSITIVE') ? 'selected' : ''; ?>>POSITIVE</option>
                                <option value="NEGATIVE" <?php echo (isset($latest_annual_test['rinne_left']) && $latest_annual_test['rinne_left'] == 'NEGATIVE') ? 'selected' : ''; ?>>NEGATIVE</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="weber_center">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['weber_center']) && $latest_annual_test['weber_center'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['weber_center']) && $latest_annual_test['weber_center'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="weber_right">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['weber_right']) && $latest_annual_test['weber_right'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['weber_right']) && $latest_annual_test['weber_right'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="weber_left">
                                <option value="">-- Select --</option>
                                <option value="YES" <?php echo (isset($latest_annual_test['weber_left']) && $latest_annual_test['weber_left'] == 'YES') ? 'selected' : ''; ?>>YES</option>
                                <option value="NO" <?php echo (isset($latest_annual_test['weber_left']) && $latest_annual_test['weber_left'] == 'NO') ? 'selected' : ''; ?>>NO</option>
                            </select>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="audiogram-table">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    <tr>
                                        <td>RIGHT</td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="250" name="right_250" value="<?php echo isset($existing_baseline['right_250']) && $existing_baseline['right_250'] !== null && $existing_baseline['right_250'] !== '' ? htmlspecialchars($existing_baseline['right_250']) : ''; ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="500" name="right_500" value="<?php echo htmlspecialchars($existing_baseline['right_500'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="1k" name="right_1k" value="<?php echo htmlspecialchars($existing_baseline['right_1k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="2k" name="right_2k" value="<?php echo htmlspecialchars($existing_baseline['right_2k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="3k" name="right_3k" value="<?php echo htmlspecialchars($existing_baseline['right_3k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="4k" name="right_4k" value="<?php echo htmlspecialchars($existing_baseline['right_4k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="6k" name="right_6k" value="<?php echo htmlspecialchars($existing_baseline['right_6k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="right" data-type="air" data-freq="8k" name="right_8k" value="<?php echo htmlspecialchars($existing_baseline['right_8k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                    </tr>
                                    <tr>
                                        <td>LEFT</td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="250" name="left_250" value="<?php echo htmlspecialchars($existing_baseline['left_250'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="500" name="left_500" value="<?php echo htmlspecialchars($existing_baseline['left_500'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="1k" name="left_1k" value="<?php echo htmlspecialchars($existing_baseline['left_1k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="2k" name="left_2k" value="<?php echo htmlspecialchars($existing_baseline['left_2k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="3k" name="left_3k" value="<?php echo htmlspecialchars($existing_baseline['left_3k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="4k" name="left_4k" value="<?php echo htmlspecialchars($existing_baseline['left_4k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="6k" name="left_6k" value="<?php echo htmlspecialchars($existing_baseline['left_6k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                        <td><input type="number" class="form-control form-control-sm baseline-input" data-ear="left" data-type="air" data-freq="8k" name="left_8k" value="<?php echo htmlspecialchars($existing_baseline['left_8k'] ?? ''); ?>" <?php echo $has_baseline ? 'readonly' : ''; ?>></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="form-section">
                <h4 class="section-title">ANNUAL AUDIOGRAPH
                    <?php if ($latest_annual_test): ?>
                        <span class="badge bg-success ms-2"><i class="fas fa-check-circle"></i> Latest Annual Test Found</span>
                    <?php endif; ?>
                </h4>
                <?php if ($latest_annual_test && !empty($latest_annual_test)): ?>
                    <div class="alert alert-success">
                        <i class="fas fa-info-circle"></i> <strong>Latest annual test found.</strong> The annual values below are displayed from the most recent annual test record (Test ID: <?php echo htmlspecialchars($latest_annual_test['id'] ?? 'N/A'); ?>, Date: <?php echo !empty($latest_annual_test['annual_date']) ? date('d/m/Y', strtotime($latest_annual_test['annual_date'])) : (!empty($latest_annual_test['examination_date']) ? date('d/m/Y', strtotime($latest_annual_test['examination_date'])) : 'N/A'); ?>).<br>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" name="annual_date" value="<?php 
                            if ($latest_annual_test && !empty($latest_annual_test['annual_date'])) {
                                echo htmlspecialchars($latest_annual_test['annual_date']);
                            } elseif ($latest_annual_test && !empty($latest_annual_test['examination_date'])) {
                                echo htmlspecialchars($latest_annual_test['examination_date']);
                            } else {
                                echo date('Y-m-d');
                            }
                        ...
                        <div class="audiogram-table">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    <tr>
                                        <td>RIGHT</td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="250" name="annual_right_250" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_250']) ? htmlspecialchars($latest_annual_test['annual_right_250']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="500" name="annual_right_500" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_500']) ? htmlspecialchars($latest_annual_test['annual_right_500']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="1k" name="annual_right_1k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_1k']) ? htmlspecialchars($latest_annual_test['annual_right_1k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="2k" name="annual_right_2k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_2k']) ? htmlspecialchars($latest_annual_test['annual_right_2k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="3k" name="annual_right_3k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_3k']) ? htmlspecialchars($latest_annual_test['annual_right_3k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="4k" name="annual_right_4k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_4k']) ? htmlspecialchars($latest_annual_test['annual_right_4k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="6k" name="annual_right_6k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_6k']) ? htmlspecialchars($latest_annual_test['annual_right_6k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="air" data-freq="8k" name="annual_right_8k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_8k']) ? htmlspecialchars($latest_annual_test['annual_right_8k']) : ''; ?>"></td>
                                    </tr>
                                    <tr>
                                        <td>LEFT</td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="250" name="annual_left_250" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_250']) ? htmlspecialchars($latest_annual_test['annual_left_250']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="500" name="annual_left_500" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_500']) ? htmlspecialchars($latest_annual_test['annual_left_500']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="1k" name="annual_left_1k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_1k']) ? htmlspecialchars($latest_annual_test['annual_left_1k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="2k" name="annual_left_2k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_2k']) ? htmlspecialchars($latest_annual_test['annual_left_2k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="3k" name="annual_left_3k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_3k']) ? htmlspecialchars($latest_annual_test['annual_left_3k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="4k" name="annual_left_4k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_4k']) ? htmlspecialchars($latest_annual_test['annual_left_4k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="6k" name="annual_left_6k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_6k']) ? htmlspecialchars($latest_annual_test['annual_left_6k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="air" data-freq="8k" name="annual_left_8k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_8k']) ? htmlspecialchars($latest_annual_test['annual_left_8k']) : ''; ?>"></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="audiogram-table">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    <tr>
                                        <td>RIGHT</td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="250" name="annual_right_bone_250" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_250']) ? htmlspecialchars($latest_annual_test['annual_right_bone_250']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="500" name="annual_right_bone_500" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_500']) ? htmlspecialchars($latest_annual_test['annual_right_bone_500']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="1k" name="annual_right_bone_1k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_1k']) ? htmlspecialchars($latest_annual_test['annual_right_bone_1k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="2k" name="annual_right_bone_2k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_2k']) ? htmlspecialchars($latest_annual_test['annual_right_bone_2k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="3k" name="annual_right_bone_3k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_3k']) ? htmlspecialchars($latest_annual_test['annual_right_bone_3k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="4k" name="annual_right_bone_4k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_4k']) ? htmlspecialchars($latest_annual_test['annual_right_bone_4k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="6k" name="annual_right_bone_6k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_6k']) ? htmlspecialchars($latest_annual_test['annual_right_bone_6k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="right" data-type="bone" data-freq="8k" name="annual_right_bone_8k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_right_bone_8k']) ? htmlspecialchars($latest_annual_test['annual_right_bone_8k']) : ''; ?>"></td>
                                    </tr>
                                    <tr>
                                        <td>LEFT</td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="250" name="annual_left_bone_250" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_250']) ? htmlspecialchars($latest_annual_test['annual_left_bone_250']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="500" name="annual_left_bone_500" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_500']) ? htmlspecialchars($latest_annual_test['annual_left_bone_500']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="1k" name="annual_left_bone_1k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_1k']) ? htmlspecialchars($latest_annual_test['annual_left_bone_1k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="2k" name="annual_left_bone_2k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_2k']) ? htmlspecialchars($latest_annual_test['annual_left_bone_2k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="3k" name="annual_left_bone_3k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_3k']) ? htmlspecialchars($latest_annual_test['annual_left_bone_3k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="4k" name="annual_left_bone_4k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_4k']) ? htmlspecialchars($latest_annual_test['annual_left_bone_4k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="6k" name="annual_left_bone_6k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_6k']) ? htmlspecialchars($latest_annual_test['annual_left_bone_6k']) : ''; ?>"></td>
                                        <td><input type="number" class="form-control form-control-sm annual-input" data-ear="left" data-type="bone" data-freq="8k" name="annual_left_bone_8k" value="<?php echo isset($latest_annual_test) && isset($latest_annual_test['annual_left_bone_8k']) ? htmlspecialchars($latest_annual_test['annual_left_bone_8k']) : ''; ?>"></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
