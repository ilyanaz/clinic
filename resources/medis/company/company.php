...
// Clear any error messages from previous redirects that shouldn't appear on company page
// Since company.php is a main landing page, clear all error messages to prevent confusion
if (isset($_SESSION['error_message'])) {
    $error_msg = $_SESSION['error_message'];
    error_log("Company page: Clearing error message: " . $error_msg);
...
if ($_POST) {
    ...
    header("Location: " . app_url('company.php'));
    exit();
}
...
<head>
    ...
    <title>Company Management - Medical Surveillance System</title>
...
<body>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="fas fa-building"></i> Company Management</h5>
                        <a href="<?php echo app_url('company_form.php'); ?>" class="btn btn-primary">
                            <i class="fas fa-plus"></i> Add New Company
                        </a>
                    </div>
                    <div class="card-body">
                        <!-- Company Statistics (Hidden) -->
                        <div class="row mb-3" style="display: none;">
                            <div class="col-md-6">
                                <div class="d-flex align-items-center">
                                    <span class="badge bg-primary fs-6 me-2">
                                        Total: <?php echo count($companies); ?> Companies
                                    </span>
                                    <span class="badge bg-success fs-6">
                                        Total Workers: <?php echo array_sum(array_column($companies, 'total_workers')); ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-2">
                            <div class="col-12">
                                <div class="text-muted">
                                    <small id="resultsCounter">Showing all companies</small>
                                </div>
                            </div>
                        </div>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody id="companyTableBody">
                                        <?php foreach ($companies as $index => $company): ?>
                                            <tr data-name="<?php echo strtolower(htmlspecialchars($company['company_name'])); ?>" 
                                                data-address="<?php echo strtolower(htmlspecialchars($company['address'])); ?>" 
                                                data-mykpp="<?php echo strtolower(htmlspecialchars($company['mykpp_registration_no'])); ?>">
                                                <td style="text-align: center;">
                                                    <?php echo $index + 1; ?>
                                                </td>
                                                <td>
                                                    <strong><?php echo htmlspecialchars(ucwords(strtolower($company['company_name']))); ?></strong>
                                                    <br>
                                                    <small class="text-muted"><?php echo htmlspecialchars(substr($company['address'], 0, 50)) . (strlen($company['address']) > 50 ? '...' : ''); ?></small>
                                                </td>
                                                <td>
                                                    <?php echo htmlspecialchars($company['district']); ?>, <?php echo htmlspecialchars($company['state']); ?>
                                                    <br>
                                                    <small class="text-muted"><?php echo htmlspecialchars($company['postcode']); ?></small>
                                                </td>
                                                <td>
                                                    <div class="contact-info-cell">
                                                        <?php if ($company['telephone']): ?>
                                                            <div class="contact-item mb-1">
                                                                ...
                                                                <span class="contact-text"><?php echo htmlspecialchars($company['telephone']); ?></span>
                                                            </div>
                                                        <?php endif; ?>
                                                        <?php if ($company['fax']): ?>
                                                            <div class="contact-item mb-1">
                                                                <i class="fas fa-fax contact-icon"></i>
                                                                <span class="contact-text"><?php echo htmlspecialchars($company['fax']); ?></span>
                                                            </div>
                                                        <?php endif; ?>
                                                        <?php if ($company['email']): ?>
                                                            <div class="contact-item">
                                                                <i class="fas fa-envelope contact-icon"></i>
                                                                <a href="mailto:<?php echo htmlspecialchars($company['email']); ?>" class="contact-email"><?php echo htmlspecialchars($company['email']); ?></a>
                                                            </div>
                                                        <?php endif; ?>
                                                ...
                                                <td>
                                                    <code><?php echo htmlspecialchars($company['mykpp_registration_no']); ?></code>
                                                </td>
                                                <td style="text-align: center;">
                                                    <?php echo $company['total_workers']; ?>
                                                </td>
                                                <td style="text-align: center;">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="<?php echo app_url('patient_list.php'); ?>?company_id=<?php echo $company['id']; ?>" class="btn btn-outline-primary" title="View List of Patients">
                                                            <i class="fas fa-clipboard-check"></i>
                                                        </a>
                                                        <a href="<?php echo app_url('company_form.php'); ?>?edit=<?php echo $company['id']; ?>" class="btn btn-outline-warning" title="Edit Company Details">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        <button type="button" class="btn btn-outline-danger" 
                                                                onclick="confirmDelete(<?php echo $company['id']; ?>, '<?php echo htmlspecialchars(ucwords(strtolower($company['company_name']))); ?>')" 
    ...
    <script>
        ...
        function updateAllWorkerCounts() {
            ...
            actionInput.value = 'update_all_worker_counts';
        ...
        function updateResultsCount(count) {
            ...
            if (count === totalCompanies) {
                counter.textContent = 'Showing all companies';
            } else {
                counter.textContent = `Showing ${count} of ${totalCompanies} companies`;
            }
        }
            ...
            if (searchInput) {
                searchInput.addEventListener('keyup', function(event) {
                    if (event.key === 'Enter') {
                        ...
                        filterCompanies(); // Real-time search
                    }
                });
